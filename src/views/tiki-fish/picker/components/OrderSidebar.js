...
const OrderSidebar = ({ scannerStatus, collapsed = false, onToggleCollapse }) => {
	...
	const activeOrder = activeOrderId === 'current' ? currentOrder : heldOrders.find((order) => order.id === activeOrderId) || currentOrder
		...
		return (
			<div className="order-sidebar collapsed">
				<div className="sidebar-header-collapsed">
					<Button color="link" size="sm" onClick={onToggleCollapse} className="collapse-toggle" title="Expand sidebar">
		...
		<div className="order-sidebar">
			<div className="sidebar-header">
				<div className="header-top">
					...
					<Button color="link" size="sm" onClick={onToggleCollapse} className="collapse-toggle d-none d-md-inline-flex" title="Collapse sidebar">
						<ChevronLeft size={18} />
					</Button>
				</div>
				...
				<div className="order-info">
					...
					{heldOrders.length > 0 && <span className="held-orders-count">{heldOrders.length} held</span>}
						...
						<div key={item.id} className="order-item">
							...
							<div className="item-details">
								...
								{!itemDiscountsVisible[item.id] && !item.discountAmount && (
									<button className="item-discount-toggle" onClick={() => toggleItemDiscount(item.id)} title="Add discount">
								...
								{item.discountAmount > 0 && !itemDiscountsVisible[item.id] && (
									<div className="item-discount-badge" onClick={() => toggleItemDiscount(item.id)}>
										<Percent size={12} />
			...
			<div className="order-summary">
				...
				<div className="order-discount-section">
					<div className="discount-header" onClick={() => setDiscountSectionCollapsed(!discountSectionCollapsed)}>
						...
						<div className="discount-controls">
							<div className="quick-discount-buttons">
								{[5, 10, 15, 20].map((percent) => (
									<button key={percent} className="quick-discount-btn" onClick={() => dispatch(setOrderDiscount('percentage', percent))}>
										{percent}%
									</button>
								))}
							</div>
							<div className="custom-discount">
								...
								{activeOrder.orderDiscount?.amount > 0 && (
									<button className="discount-clear" onClick={() => dispatch(setOrderDiscount('percentage', 0))} title="Clear discount">
										Clear
									</button>
								)}
							</div>
				...
				{activeOrder.totalDiscount > 0 && (
					<div className="summary-row total-discount-row">
						...
						<span className="value discount">
							-{formatPrice(activeOrder.totalDiscount)}({activeOrder.discountPercentage?.toFixed(1)}%)
						</span>
					</div>
				)}
				...
				<div className="payment-methods">
					<div className="payment-header" onClick={() => setPaymentMethodCollapsed(!paymentMethodCollapsed)}>
				...
				<div className="order-actions">
					<button className="action-btn info" onClick={() => setShowHoldOrderManager(true)}>
						<Archive size={16} />
						Manage Held Orders
						{heldOrders.length > 0 && (
					...
					<div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
						<button className="action-btn secondary" onClick={handleHoldOrder} disabled={activeOrder.items.length === 0} style={{ flex: 1 }}>
							Hold Order
						</button>
						<button className="action-btn primary" onClick={handlePlaceOrder} disabled={activeOrder.items.length === 0} style={{ flex: 1 }}>
							Place Order
						</button>
					</div>
				</div>
				...
				{activeOrder.items.length > 0 && (
					<button className="action-btn secondary" onClick={handleClearOrder} style={{ marginTop: '0.5rem', width: '100%' }}>
						Clear Order
					</button>
				)}
			</div>
			...
			<HoldOrderManager isOpen={showHoldOrderManager} toggle={() => setShowHoldOrderManager(false)} />
		</div>
	)
}
