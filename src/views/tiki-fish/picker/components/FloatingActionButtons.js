		...
		<>
			...
			<div className={`fab-container ${fabExpanded ? 'expanded' : ''}`}>
				...
				<div className={`fab-secondary ${fabExpanded ? 'visible' : ''}`}>
					{/* Search Toggle */}
					<Button color={searchVisible ? 'primary' : 'secondary'} className="fab-secondary-btn" id="fab-search" onClick={onToggleSearch}>
						<Search size={18} />
					</Button>
					...
					{/* Filter Toggle */}
					<Button color={filterVisible ? 'primary' : 'secondary'} className="fab-secondary-btn" id="fab-filter" onClick={onToggleFilter}>
						<Filter size={18} />
					</Button>
					...
					{getTotalItems() > 0 && (
						<>
							<Button color="info" className="fab-secondary-btn" id="fab-hold" onClick={handleHoldOrder}>
								<Menu size={18} />
							</Button>
					...
					{getTotalItems() > 0 && (
						<>
							<Button color="success" className="fab-secondary-btn" id="fab-place-order" onClick={handlePlaceOrder}>
				...
				{/* Main FAB Button */}
				<Button color="primary" className="fab-main" onClick={toggleFab} id="fab-main">
			...
			{getTotalItems() > 0 && (
				<div className="fab-order-summary">
					<div className="order-summary-content">
						...
						<div className="summary-total">{String(formatPrice(getOrderTotal()))}</div>
						{heldOrders.length > 0 && (
