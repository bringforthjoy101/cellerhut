...
import { Button, FormGroup, Label, Form, Input, Row, Col, FormFeedback, Modal, ModalHeader, ModalBody, ModalFooter } from 'reactstrap'
...
const SupplyModal = ({ open, toggleSidebar, selectedSupply }) => {
	// ** State
	...
	useEffect(() => {
		if (selectedSupply) {
			setData({
				...
				items:
					selectedSupply.supply_items?.map((item) => ({
	...
	return (
		<Modal isOpen={open} toggle={toggleSidebar} className="modal-dialog-centered modal-xl" backdrop="static" keyboard={false}>
			<ModalHeader toggle={toggleSidebar}>{selectedSupply ? 'Edit Supply' : 'New Supply'}</ModalHeader>
						...
						{data.items.length > 0 ? (
							<>
								<div className="d-none d-md-block">
									<Row className="supply-items-header border-bottom pb-2 mb-2">
										<Col md={5}>
											<small className="fw-bold">Product</small>
										</Col>
										...
										<Col md={1} className="text-center">
											<small className="fw-bold">VAT %</small>
										</Col>
										...
										<div key={index} className={`supply-item mb-2 p-2 ${index % 2 === 0 ? 'bg-light' : ''} rounded`}>
											<Row className="align-items-center">
												<Col xs={12} md={5} className="mb-2 mb-md-0">
													...
													/>
													{errors[`item_${index}_productId`] && <FormFeedback className="d-block">{errors[`item_${index}_productId`]}</FormFeedback>}
												</Col>
												<Col xs={6} md={1} className="mb-2 mb-md-0">
													...
													/>
													{errors[`item_${index}_quantity`] && <FormFeedback>{errors[`item_${index}_quantity`]}</FormFeedback>}
												</Col>
												<Col xs={6} md={2} className="mb-2 mb-md-0">
													...
													{errors[`item_${index}_unitPrice`] && <FormFeedback className="d-block">{errors[`item_${index}_unitPrice`]}</FormFeedback>}
												</Col>
												<Col xs={6} md={1} className="mb-2 mb-md-0">
													<Label for={`vatRate-${index}`} className="d-md-none">
														VAT %
													</Label>
													<Input
														type="number"
												...
												<Col xs={12} md={1} className="text-center text-md-end">
													{data.items.length > 1 && (
														<Button color="danger" size="sm" className="btn-icon" onClick={() => removeItem(index)} title="Remove item">
															<Trash size={14} />
														</Button>
													)}
												</Col>
